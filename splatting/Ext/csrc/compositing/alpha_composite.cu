...
__global__ void alphaCompositeCudaForwardKernel(
  ...
  const int64_t C = features.size(1); //重要修改，这个文件中还有三处
  ...
  for (int pid = tid; pid < num_pixels; pid += num_threads) {
    ...
    atomicAdd(&result[batch][3][j][i], 1);
    ...
    for (int k = 0; k < points_idx.size(1); ++k) {
      ...
      float alpha = alphas[batch][k][j][i] * features[n_idx][3];
      ...
      // 颜色混合
      atomicAdd(&result[batch][0][j][i], (features[n_idx][0] - result[batch][0][j][i]) * alpha);
      atomicAdd(&result[batch][1][j][i], (features[n_idx][1] - result[batch][1][j][i]) * alpha);
      atomicAdd(&result[batch][2][j][i], (features[n_idx][2] - result[batch][2][j][i]) * alpha);
      atomicAdd(&result[batch][3][j][i], (1 - result[batch][3][j][i])* alpha);
    }
  }
}
  ...
  for (int pid = tid; pid < num_pixels; pid += num_threads) {
    ...
    for (int k = 0; k < points_idx.size(1); ++k) {
      ...
      atomicAdd(
          ...
          cum_alpha * features[n_idx][ch]* grad_outputs[batch][ch][j][i]);
      atomicAdd(
          &grad_features[n_idx][ch],
      ...
      for (int t = 0; t < k; ++t) {
        ...
        atomicAdd(
            ...
            -grad_outputs[batch][ch][j][i] * features[n_idx][ch] * cum_alpha *
...
at::Tensor alphaCompositeCudaForward(
  ...
  alphaCompositeCudaForwardKernel<<<numBlocks, threadsPerBlock, 0, stream>>>(
      ...
      result.packed_accessor64<float, 4, at::RestrictPtrTraits>(), //包含维度信息
